...
def create_vector_db(folder_name):
  ...
  for document in loader.load_and_split(text_splitter=text_splitter):
    processed_pages += 1
    if vec_db is None:
...
def vectorization_func(params: dict):
    ...
    # Try creating folders; handle FileExistsError
    try:
        ...
        # Handle case where the folder already exists
        logger.info(f"Folder '{folder_name}' already exists inside '{temp_dir}'.") 

    # Download the file from Azure Blob Storage to the specified folder
